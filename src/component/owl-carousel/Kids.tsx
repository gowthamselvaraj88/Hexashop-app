...
export default function Owlcarosuel1(){
    ...
    const kidsheading =[
        {
            id:0,
            menname:"Kid's Latest",
            mendescribe:"Details to details is what makes Hexashop different from the other themes."
        }
    ];
    
    const Scrollheading = kidsheading.map((item) => (
        <Heading MenTitle={item.menname} MansDescribe={item.mendescribe} />
      ));

    return(
        <>
        <section id='kids' className='container mx-auto px-4'>
                {Scrollheading}
                <div className='grid grid-cols-1 md:px-20 lg:px-32'>
